...
export function attachWebpackHooks(plugin) {
  ...
  const attachCssExtractionHook = (compiler) => {
      ...
      compilation.hooks.processAssets.tap(tapOptions, (assets) => {
        ...
        if (cssAssets.length === 0) {
          compilation.warnings.push(
            new Error('[stylex] No CSS asset found to inject into. Skipping.'),
          );
          return;
        }
  ...
  return {
    ...plugin,
    webpack: attachCssExtractionHook,
    rspack: attachCssExtractionHook,
  };
}
